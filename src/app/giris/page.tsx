    ...
    <div className="w-full max-w-md">
      ...
      <form onSubmit={submit} className="space-y-5">
        <div className="flex flex-col gap-1.5">
          ...
          <input type="email" value={form.email} onChange={e => set("email", e.target.value)} placeholder="ornek@email.com" required className="border border-light-gray bg-cream px-4 py-3 text-sm text-charcoal placeholder:text-warm-gray/40 focus:outline-none focus:border-charcoal transition-colors" />
        </div>
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between">
            <label className="text-[10px] tracking-ultra font-medium text-charcoal/60">ŞİFRE</label>
            <button type="button" className="text-[10px] text-gold hover:text-gold-light transition-colors underline underline-offset-2">Şifremi Unuttum</button>
          </div>
          <div className="relative">
            <input type={showPass ? "text" : "password"} value={form.password} onChange={e => set("password", e.target.value)} placeholder="••••••••" required className="w-full border border-light-gray bg-cream px-4 py-3 pr-11 text-sm text-charcoal placeholder:text-warm-gray/40 focus:outline-none focus:border-charcoal transition-colors" />
        ...
        <button type="submit" disabled={loading} className="w-full bg-charcoal text-cream py-4 text-[11px] tracking-ultra font-medium hover:bg-charcoal/85 transition-colors duration-300 disabled:opacity-50 flex items-center justify-center gap-2 mt-2">
      ...
      <p className="text-sm text-warm-gray font-light text-center mt-8">
        Hesabınız yok mu?{" "}
        <Link href="/kayit" className="text-charcoal font-medium underline underline-offset-2 hover:text-gold transition-colors">Üye olun</Link>
      </p>
    ...
    <div className="min-h-[calc(100vh-100px)] bg-cream flex">
      <div className="hidden lg:flex lg:w-1/2 bg-[#18160f] flex-col items-center justify-center p-16 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-sm">
          ...
          <h2 className="text-3xl font-serif font-medium text-white leading-tight mb-6">Stilinizi<br />keşfetmeye<br />devam edin</h2>
          <p className="text-sm text-white/40 font-light leading-relaxed">Siparişlerinizi takip edin, adres bilgilerinizi yönetin ve özel kampanyalardan ilk siz haberdar olun.</p>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <Suspense fallback={<div className="w-6 h-6 border border-charcoal/20 border-t-charcoal rounded-full animate-spin" />}>
          <GirisForm />
        </Suspense>
      </div>
    </div>
