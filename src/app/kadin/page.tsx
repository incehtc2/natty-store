...
export default async function KadinPage({ searchParams }: { searchParams: Promise<{ tip?: string }> }) {
  const { tip } = await searchParams;
  const products = await getProducts({ category: "kadin" });
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <h1 className="text-5xl md:text-6xl font-serif font-medium tracking-wide text-charcoal leading-none">
                Kadın{tip === "ayakkabi" ? " · Ayakkabı" : tip === "canta" ? " · Çanta" : ""}
              </h1>
            </div>
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-2">
                {[
                  { label: "TÜMÜ", href: "/kadin", active: !tip },
                  { label: "AYAKKABI", href: "/kadin?tip=ayakkabi", active: tip === "ayakkabi" },
                  { label: "ÇANTA", href: "/kadin?tip=canta", active: tip === "canta" },
