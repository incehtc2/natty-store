...
export default async function KampanyaPage() {
  const products = await getProducts({ kampanya: true });
  const kadin = products.filter(p => p.category === "kadin");
  const erkek = products.filter(p => p.category === "erkek");
    ...
    <div className="min-h-screen bg-cream">
      <div className="bg-[#18160f] py-20 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto px-6">
          ...
          <p className="text-sm text-white/40 font-light mb-8">Seçili ürünlerde %30&apos;a varan indirim <br /> Stoklar sınırlıdır</p>
        </div>
      </div>
        ...
        {kadin.length > 0 && (
          <section className="mb-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-5 gap-y-10">
              {kadin.map(p => <ProductCard key={p.id} product={{ ...p, originalPrice: p.original_price ?? undefined, isNew: p.is_new, badge: p.badge ?? undefined }} />)}
            </div>
          </section>
        )}
        {erkek.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-5 gap-y-10">
              {erkek.map(p => <ProductCard key={p.id} product={{ ...p, originalPrice: p.original_price ?? undefined, isNew: p.is_new, badge: p.badge ?? undefined }} />)}
            </div>
          </section>
        )}
        <div className="mt-20 border border-light-gray p-8 flex flex-col sm:flex-row gap-8 items-start">
          {[{ t: "Ücretsiz Kargo", d: "Kampanyalı ürünlerde tüm siparişlere ücretsiz kargo." }, { t: "30 Gün İade", d: "Beğenmediğiniz ürünü 30 gün içinde iade edebilirsiniz." }, { t: "Stok Tükenmeden", d: "Kampanya ürünleri sınırlı stokla sunulmaktadır." }].map(({ t, d }) => (
