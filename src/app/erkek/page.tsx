...
export default function ErkekPage() {
  const ayakkabi = products.filter(p => p.category === "erkek" && p.type === "ayakkabi");
  const canta = products.filter(p => p.category === "erkek" && p.type === "canta");
  const kampanya = products.filter(p => p.category === "erkek" && p.originalPrice);
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10 py-16 md:py-20">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <h1 className="text-5xl md:text-6xl font-serif font-medium tracking-wide text-charcoal leading-none">Erkek</h1>
            </div>
